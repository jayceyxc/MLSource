...
@file: number_classify.py
@time: 15/11/2017 09:32
"""
...
def train():
    ...
    with tf.name_scope('model') as scope:
        with tf.name_scope('fc1') as scope:
            """
            神经网络的第一层，将输入转换成第一层隐藏层的输入
            fc1 stands fro 1st fully connected layer
        ...
        with tf.name_scope('fc2') as scope:
            """
            神经网络的第一层隐藏层，将第一层的输出转变成第二层隐藏层的输入
    ...
    with tf.name_scope('train') as scope:
        """
        定义损失函数，及模型训练时的参数优化函数
        """
        with tf.name_scope('loss') as scope:
            # loss function 定义损失函数
            cross_entropy = tf.nn.softmax_cross_entropy_with_logits(labels=labels, logits=logits)
        # use adam optimizer for training with a learning rate of 0.001 进行参数优化
        train_step = tf.train.AdamOptimizer(0.001).minimize(cross_entropy)

    with tf.name_scope('evaluation') as scope:
        # evaluation 定义模型评估的函数
        correct_prediction = tf.equal(tf.argmax(logits, 1), tf.argmax(labels, 1))
        # reduce_mean 计算均值，当不指定axis时，计算所有元素的均值，如果指定axis值，则计算指定坐标轴上的均值
    ...
    with tf.Session() as sess:
        ...
        for step in range(20000):
            ...
            if step % 100 == 0:
                ...
                # add merged summaries to filewriter, so they are saved to disk
        ...
        print(sess.run(labels, feed_dict={x:[mnist.test.images[0]], labels: [mnist.test.labels[0]]}))
